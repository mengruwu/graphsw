...
parser.add_argument('--ls_weight', type=float, default=0.2, help='weight of LS regularization')
...
parser.add_argument('--ls_turn_up', type=bool, default=True, help='save default model')
...
def env_by_item(args, trn_info):
	...
	for epoch in range(5):
		...
		args.epoch = epoch
		for hop in [1,2]:
			if hop == 1: tags_number = [0,1]
			else: tags_number = [2,3]
			args.n_iter = hop
			selc_data_and_run(load_pretrain_emb = False, tags = tags_number[1], show_topk = show_topk_mode)

			# stage_wise traing
			if args.stage_wise == True:
				for repeat in range(20):
					selc_data_and_run(load_pretrain_emb = True, tags = tags_number[1], show_topk = show_topk_mode)
...
if __name__ == "__main__":
	...
	tags = ['h1 no ','h1 no emb','h2 no','h2 no emb']

	# stage_wise traing
	args.stage_wise = True
	trn_info = Train_info_record_emb_sw_ndcg(args,tags)
