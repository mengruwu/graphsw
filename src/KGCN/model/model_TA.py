...
class KGCN(object):
    ...
    def _parse_args(self, args, adj_entity, adj_relation, interaction_table, offset):
        ...
        self.pretrained_embeddings_action = f"../../data/data_filter/{args.dataset}/emb/rela_tr_emb_test_{self.save_model_name}.npy"
        self.pretrained_embeddings_entity = f"../../data/data_filter/{args.dataset}/emb/enti_tr_emb_test_{self.save_model_name}.npy"
        self.pretrained_embeddings_user = f"../../data/data_filter/{args.dataset}/emb/user_tr_emb_test_{self.save_model_name}.npy"
    ...
    def save_pretrain_emb_fuc(self,sess):
        ...
        np.save(f"../../data/data_filter/{self.dataset}/emb/enti_tr_emb_test_{self.save_model_name}.npy", enti_emb)

        rela_emb = sess.run(self.relation_emb_matrix)
        np.save(f"../../data/data_filter/{self.dataset}/emb/rela_tr_emb_test_{self.save_model_name}.npy", rela_emb)

        user_emb = sess.run(self.user_emb_matrix)
        np.save(f"../../data/data_filter/{self.dataset}/emb/user_tr_emb_test_{self.save_model_name}.npy", user_emb)
    ...
    def aggregate(self, entities, relations):
        ...
        # print('relation_vectors shape = ', relation_vectors[0].get_shape())
